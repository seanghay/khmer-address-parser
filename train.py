...
def main():
  ...
  train_samples = []
  for _ in range(3):
    train_samples += list(generate_address(
  ...
  for epoch in epoch_pbar:
    ...
    epoch_pbar.set_postfix(loss=f"{loss:.4f}", P=f"{metrics['precision']:.4f}", R=f"{metrics['recall']:.4f}", F1=f"{val_f1:.4f}")
    tqdm.write(f"Epoch {epoch:3d} | loss={loss:.4f} | P={metrics['precision']:.4f} R={metrics['recall']:.4f} F1={val_f1:.4f}")
